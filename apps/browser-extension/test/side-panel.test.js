...
// --- registry sanity ---------------------------------------------------------
check('registry: 25 platforms', windowMock.RASTCHIN_PLATFORMS.length, 25);
...
check('registry: Google Search host matches',
    windowMock.rastchinMatchPlatformFromUrl('https://www.google.com/search?q=%D9%81%D8%A7%D8%B1%D8%B3%DB%8C')?.id, 'googleSearch');
check('registry: Maps is not claimed by Google Search',
    windowMock.rastchinMatchPlatformFromUrl('https://maps.google.com/'), null);
...
check('init: total metric in Persian digits', element('totalPlatformCount').textContent, '۲۵');
check('init: active metric counts 24 of 25', element('activePlatformCount').textContent, '۲۴');
check('init: version badge from manifest', element('panelVersion').textContent, `v${MANIFEST_VERSION}`);
check('init: platform grid rendered', element('platformGrid').children.length, 25);
{
...
// --- settings tab ----------------------------------------------------------------
{
    ...
    check('settings: 25 switches rendered', rows.length, 25);
    ...
    check('settings: active metric drops after disable', element('activePlatformCount').textContent, '۲۴');
...
check('storage sync: active metric reflects external change', element('activePlatformCount').textContent, '۲۳');
