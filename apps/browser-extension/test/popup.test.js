...
check('total platform count is dynamic', element('totalPlatformCount').textContent, '24');
check('active platform count reflects settings', element('activePlatformCount').textContent, '22');
...
check('active count updates after popup toggle', element('activePlatformCount').textContent, '23');

storageChanges[0]({ claudeEnabled: { newValue: false } }, 'sync');
check('active count updates after external settings change', element('activePlatformCount').textContent, '22');
...
check('active count returns after reopening global gate', element('activePlatformCount').textContent, '22');
