...
OUT="$APP_ROOT/unpacked"

[ "$OUT" = "$APP_ROOT/unpacked" ] || { echo "ERROR: OUT safety check failed" >&2; exit 1; }
...
cp "$APP_ROOT/manifest.json" "$OUT/manifest.json"
...
echo "✓ unpacked/ ready — $FILE_COUNT files"
